        ...
        initialize: function (freshApi, freshState, initializeCallback) {
			...
			$(document).ready(function () {
				$.ajax({
					...
					data: { userName: session.userName, dbName: session.database, serverName: server, sessionId: session.sessionId },
			...
			*/
			...
			api.getSession(function (session,server) {
				...
				$.ajax({
					type: 'PUT',
					dataType: 'json',
					url: 'https://localhost:44355/api/Geotab/SaveEmployeeRecord',
					data: { userName: session.userName, dbName: session.database, serverName: server, sessionId: session.sessionId },
					success: function (Data) {
						window.alert(Data.userName);
					},
